...
1. formatNumber 유틸 함수 분리

2. 페이지 컴포넌트 내부 컴포넌트 분리 -> 관성적으로 분리하는것인가에 대한 의문
   - 노션에 명시된 기능만 구현한다면 하나의 파일에 전부 구현해도 될거 같으나 서비스의 유지보수나 장기적인 확장성을 고려한 설계, 추상화 관점에서 구현한다면 파일을 나누는게 맞다고 생각했습니다.
   - 입력 영역을 SavingsCalculatorInputs 컴포넌트로 분리하고 폼 상태와 핸들러를 useSavingsForm 훅으로 추출하여 Page에서는 값과 핸들러만 연결하도록 구성했습니다.
   - 탭 영역은 SavingsProductList와 CalculationResult로 분리했습니다. 현재 코드량으로는 분리가 불필요해 보이지만 추후 기능 추가를 대비해 나누었습니다.
   - 상품 필터링, 추천 상품 정렬, 이자 계산 등 비즈니스 로직은 useSavingsCalculator 훅으로 추출하여 UI와 로직을 분리했습니다
   - 컴포넌트 이름과 props만 보고 해당 컴포넌트가 무슨 역할을 하는지 알 수 있게 나누었습니다
...
펼쳐보기
본질은 무엇인가 -> 찾기
본질을 나타내는 것을 아닌것들을 덜어내면서 찾기
본질을 나타내는 것들만 냄겨두기
그 이후에 그 본질을 나타내는 것들을 어떻게 보여줄지 고민하기

우리는 왜 확장 가능성을 고려하는가?

우리는 마땅히 소프트웨어를 만들어야 한다
소프트웨어는 수정하기 쉬워서 소프트웨어임
우리는 왜 이렇게 변경에 두려워하고 불안해하는가
답은 하나다 -> 추상화를 잘 못하고, 코드를 가위질 해놓고 엮어 놓기에 알아볼 수 없고, 꼬여 있어서 고치기 힘든 것이다.
코드를 가위질하지 않고, 꼬아놓지 않으면 고치기 쉬워지니까 변경에 잘 대응할 수 있다
그러면 확장도 잘 되고 유지보수도 잘 될 수 밖에 없는것이다
변경 대응이 잘 되기에 확장성이 높아질 수 밖에 없다

추상화 단위를 맞추자 -> 레벨 맞추자

무작정 변수로 만들지 말고 일단 펼처놓고 얜 뭐하는 앤가 살펴보자
단계별로 살펴보기 흐름을 보자

즉시실행함수도 써보면서 일단 펼처놓아라
근본적으로 의문가지면서

목표금액 > 0이 아니라
=> 사용자가 목표금액을 입력하지 않았을 때라는 의미가 보여야 함
...
-> 어떻게 드러나야 하지?
-> 데이터가 있는데
-> 조건에 따라서 어떤한 연산을 걸거나 안걸거나

조건 계산은 세부사항임
위 내용이 더 큰 애여

how (a-b, 세부사항)가 나타나느게 아니라,
what(sortByRateDecs, 본질)으로 나타내야 함
