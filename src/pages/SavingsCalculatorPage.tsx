...
import { isMonthlyAmountInRange, isTabEnum } from "domains/savings-product";
...
function SavingsCalculatorPage() {
	...
	const [selectedSavingsProduct, setSelectedSavingsProduct] =
	...
	return (
		<>
			...
			<NumberField
				...
				suffix="원"
				value={
					filters.monthlyAmount == null ? undefined : filters.monthlyAmount
				}
				onValueChange={(value) => {
					updateFilters((prev) => ({
						...prev,
			...
			<SelectBottomSheet
				label="저축 기간"
				title="저축 기간을 선택해주세요"
				value={filters.term == null ? undefined : filters.term}
				onChange={(value) => {
					updateFilters((prev) => ({ ...prev, term: value }));
				}}
			>
										...
										return (
											<>
												{filterSavingsProducts(savingsProducts, {
													...
													const isSelected =
														selectedSavingsProduct?.id === savingsProduct.id;
													return (
											...
											<>
												{selectedSavingsProduct ? (
													<CalculationResult
														...
														savingsProducts={filterSavingsProducts(
															savingsProducts,
															{
																monthlyAmount: filters.monthlyAmount,
																term: filters.term,
...
function filterSavingsProducts(
	...
	return savingsProducts
		.filter((savingsProduct) =>
			isMonthlyAmountInRange(savingsProduct, defaultTo(monthlyAmount, 0)),
		)
		.filter((savingsProduct) => term === savingsProduct.availableTerms);
}
