...
const CalculationResult = ({
	...
	const expectedProfitAmount =
		defaultTo(monthlyAmount, 1) *
		defaultTo(term, 1) *
		get반기예상연이자율(selectedSavingsProduct.annualRate);

	const differenceFromGoalAmount =
		defaultTo(goalAmount, 0) - expectedProfitAmount;

	const recommendedMonthlyAmount =
		defaultTo(goalAmount, 0) /
		(defaultTo(term, 1) *
			get반기예상연이자율(selectedSavingsProduct.annualRate));
	...
	return (
		<>
			...
			/>
			<Spacing size={12} />
			{take(orderBy(savingsProducts, ["annualRate"], ["desc"]), 2).map(
				(savingsProduct) => {
					return (
