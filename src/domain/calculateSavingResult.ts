export const calculateExpectedProfit = (monthlyAmount: number, term: number, annualRate: number): number => {
  return monthlyAmount * term * (1 + annualRate * 0.01 * 0.5);
};

export const calculateTargetAmountDifference = (targetAmount: number, expectedIncome: number): number => {
